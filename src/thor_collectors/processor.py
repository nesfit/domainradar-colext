...
from kafka_multiprocessor import AsyncKafkaMessageProcessor, Message, SimpleMessage
...
class BaseAsyncCollectorProcessor(
    AsyncKafkaMessageProcessor[TCollectorKey, TCollectorRequest], abc.ABC
):
    def __init__(
        self,
        config: dict,
        component_name: str,
        output_topic: str,
        key_class: Type[TCollectorKey],
        value_class: Type[TCollectorRequest],
        result_class: Type[TCollectorResponse],
    ):
    ...
    def process_error(
        self, message: Message[TCollectorKey, TCollectorRequest], error: BaseException | int
    ) -> list[SimpleMessage]:
        if isinstance(error, BaseException):
            self._logger.k_unhandled_error(error, message.key)
            res = self._make_result(error, message.key_raw)
        elif (
            error == self.ERROR_RATE_LIMITED_IMMEDIATE
            or error == self.ERROR_RATE_LIMITED_WITH_TIMEOUT
        ):
            self._logger.k_debug("Rate limited", message.key)
            status_code = (
                rc.LOCAL_RATE_LIMIT
                if error == self.ERROR_RATE_LIMITED_IMMEDIATE
                else rc.LRL_TIMEOUT
            )
    ...
    def _make_result(
        self, error: BaseException | str | None, key_raw: bytes, code: int | None = None
        ...
        try:
            if is_ip_result:
                result = self._result_class(
                    status_code=code, error=error_msg, collector=self._collector_name
                )
            else:
    ...
    def _should_omit_ip(self, dn_ip: IPToProcess, request: IPProcessRequest | None) -> bool:
        result = (
            request is not None
            and request.collectors is not None
            and self._collector_name not in request.collectors
        )
        if result:
