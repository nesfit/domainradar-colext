 ...
 */
package cz.vut.fit.domainradar.log4j;
 ...
 */
@Plugin(name = "KafkaPatched", category = Node.CATEGORY, elementType = Appender.ELEMENT_TYPE, printObject = true)
public final class KafkaPatchedAppender extends AbstractAppender {
    ...
    public static class Builder<B extends Builder<B>> extends AbstractAppender.Builder<B>
            implements org.apache.logging.log4j.core.util.Builder<KafkaPatchedAppender> {
        ...
        @Override
        public KafkaPatchedAppender build() {
            final Layout<? extends Serializable> layout = getLayout();
            if (layout == null) {
                AbstractLifeCycle.LOGGER.error("No layout provided for KafkaAppender");
                return null;
            }
            final KafkaPatchedManager kafkaManager = KafkaPatchedManager.getManager(
                    ...
                    getPropertyArray(),
                    key);
            return new KafkaPatchedAppender(
                    getName(),
                    layout,
                    getFilter(),
                    isIgnoreExceptions(),
    ...
    @Deprecated
    public static KafkaPatchedAppender createAppender(
        ...
        final KafkaPatchedManager kafkaManager =
                KafkaPatchedManager.getManager(configuration.getLoggerContext(), name, topic, true, properties, key);
        return new KafkaPatchedAppender(name, layout, filter, ignoreExceptions, kafkaManager, null, 0);
    }
    ...
    private final KafkaPatchedManager manager;

    private KafkaPatchedAppender(
            final String name,
            final Layout<? extends Serializable> layout,
            final Filter filter,
            final boolean ignoreExceptions,
            final KafkaPatchedManager manager,
            final Property[] properties,
    ...
    private void tryAppend(final LogEvent event) throws ExecutionException, InterruptedException, TimeoutException {
        ...
        manager.send(data, event.toImmutable());
    }
}
