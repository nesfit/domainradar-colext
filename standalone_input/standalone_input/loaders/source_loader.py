...
from standalone_input.loaders.utils import LoaderUtils as Utils, create_getter, create_mapper
import logging
...
class SourceLoader:
        ...
        with open(filename, "r") as f:
            for line in f:
                ...
                if line.startswith(Utils.comment_prefixes) or len(line) == 0:
                    continue
                self.sources.append(Source(url=line,
                                           category='unknown',
    ...
    @staticmethod
    def _get_urlhaus(file: str, source: Source) -> list[Domain]:
        domains: List[Domain] = []
    ...
    @staticmethod
    def _get_txt(file: str, source: Source) -> list[Domain]:
        domains: List[Domain] = []
