...
from .types import SimpleMessage, Offset, ROB, ROBContainer, ProcessedEntry
...
class WorkerManager:
    def __init__(
        ...
        self._liveliness_check_interval: float = self._client_config.get(
            "liveliness_check_interval", 10.0
        )
        self._next_liveliness_check: float = time.time() + self._liveliness_check_interval
        self._entry_late_warning_threshold: float = self._client_config.get(
            "entry_late_warning_threshold", 30.0
        )
        self._max_entry_time_in_queue: float = self._client_config.get(
            "max_entry_time_in_queue", 120.0
        )
        self._max_queued_items: int = self._client_config.get("max_queued_items", 100)
        self._resume_after_freed_items: int = self._client_config.get(
    ...
    def handle_message(self, msg: ck.Message):
        ...
        self._in_flight += 1
        self._logger.trace(
            "Late warning for p=%s, o=%s at %s; in_flight=%s; adding to_process entry",
            partition,
            offset,
            late_warning,
            self._in_flight,
    ...
    def _update_rob(self) -> None:
        ...
        while livelock_prevention_counter < self._max_entries_to_confirm:
            ...
            self._logger.trace(
                "Accepted item with %s messages at ctr=%s, p=%s, o=%s",
                len(processed[2]),
                livelock_prevention_counter,
                processed[0],
                processed[1],
            )
    ...
    def _produce_results(self) -> None:
        ...
        for partition, offsets in self._robs_for_partitions.items():
            ...
            self._logger.trace(
                "Evaluating ROB for p=%s at t=%s; offsets in ROB: %s",
                partition,
                current_time,
                len(offsets),
            )
            while len(offsets) > 0:
                ...
                if item_done:
                    self._logger.trace(
                        "Top of ROB p=%s is o=%s, done with %s msgs",
                        partition,
                        offset,
                    ...
                    try:
                        ...
                        self._in_flight -= 1
                        self._logger.trace(
                            "Popped offset, %s offsets remaining; in_flight=%s",
                            len(offsets),
                            self._in_flight,
                        )
                    except KafkaException as e:
                        self._logger.error(
                            "Error commiting transaction for p=%s, o=%s",
                            partition,
                            offset,
                            exc_info=e,
                        )
                        ...
                        if (
                            kafka_error_code == KafkaError.PRODUCER_FENCED
                    ...
                    self._logger.trace(
                        "Top of ROB p=%s is o=%s, waiting with exp_t=%s",
                        partition,
                        offset,
                    ...
                    if (
                        not item_done
                        and expire_time_or_result < current_time
                        ...
                        if (current_time - expire_time_or_result) > self._max_entry_time_in_queue:
                            self._logger.error(
                                "Commiting suicide as an entry took over %s s in queue",
                    ...
                    self._logger.trace(
                        "Breaking loop for ROB p=%s; %s offset remaining", partition, len(offsets)
                    )
                    break
    ...
    def close(self) -> None:
        ...
        for process in self._workers:
            ...
            try:
                ...
                self._logger.debug(
                    "ProcessLookupError for %s - process already dead?", process.pid
                )
                pass
    ...
    def _make_worker(self, worker_id: int):
        ...
        self._logger.info(
            "Started worker process %s (PID %s)", worker_process.name, worker_process.pid
        )
        ...
        for process in self._workers:
            if not process.is_alive():
                self._logger.warning(
                    "Worker process %s (PID %s) is dead", process.name, process.pid
                )
