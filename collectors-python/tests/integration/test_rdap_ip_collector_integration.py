...
from thor_collectors.rdap_ip.rdap_ip import RDAPIPProcessor
...
def _run_client(config_path: str, input_topic: str):
    ...
    run_client(input_topic, RDAPIPProcessor)


@pytest.mark.integration
def test_rdap_ip_collector_end_to_end(tmp_path):
    ...
    if output_topic != "collected_IP_data":
        pytest.skip("KAFKA_OUTPUT_TOPIC must be collected_IP_data for RDAPIP integration")

    config_path = write_kafka_config(
        tmp_path,
        brokers,
        "rdap-ip-collector-test",
        [
            "[rdap-ip]",
            "http_timeout = 5",
        ],
    ...
    consumer = ck.Consumer(
        {
            ...
            "group.id": "rdap-ip-collector-consumer",
