...
class DNSCollectorOptions:
    ...
    This class is used to store the various options that can be configured for the zone
    and DNS collectors.
    ...
    Attributes:
        ...
        types_to_process_IPs_from (list): The types of DNS records to process IPs from.
                                          Not used in the zone collector.
        max_record_retries (int): The maximum number of retries for each record.
                                  Not used in the zone collector.
    """
    ...
    def from_config(cls, component_config):
        ...
        """
        dns_servers = component_config.get(
            "dns_servers", ["195.113.144.194", "193.17.47.1", "195.113.144.233", "185.43.135.1"]
        )
        timeout = component_config.get("timeout", 5)
        rotate_nameservers = component_config.get("rotate_nameservers", False)
        types_to_scan = component_config.get(
            "types_to_scan", ["A", "AAAA", "CNAME", "MX", "NS", "TXT"]
        )
        types_to_process_ips_from = component_config.get(
        ...
        return cls(
            ...
            types_to_process_IPs_from=types_to_process_ips_from,
    ...
    def __init__(self, **kwargs):
        ...
        Args:
            **kwargs: Arbitrary keyword arguments used to set the options for the zone or
                      DNS collector. If an option is not specified, the default value is None.
                      Refer to the class documentation for the list of available options.
        """
