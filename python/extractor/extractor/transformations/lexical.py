...
class LexicalTransformation(Transformation):
    ...
    def transform(self, df: DataFrame) -> DataFrame:
        ...
        df["lex_malware_bigram_matches"] = df["tmp_concat_subdomains"] \
            .apply(find_ngram_matches, args=(self._malware_ngram_freq["bigram_freq"],))
        df["lex_malware_trigram_matches"] = df["tmp_concat_subdomains"].apply(find_ngram_matches, args=(
            self._malware_ngram_freq["trigram_freq"],))
        df["lex_malware_tetragram_matches"] = df["tmp_concat_subdomains"].apply(find_ngram_matches, args=(
            self._malware_ngram_freq["tetragram_freq"],))
        df["lex_dga_bigram_matches"] = df["tmp_concat_subdomains"] \
            .apply(find_ngram_matches, args=(self._dga_ngram_freq["bigram_freq"],))
        df["lex_dga_trigram_matches"] = df["tmp_concat_subdomains"] \
            .apply(find_ngram_matches, args=(self._dga_ngram_freq["trigram_freq"],))
        df["lex_dga_tetragram_matches"] = df["tmp_concat_subdomains"] \
            .apply(find_ngram_matches, args=(self._dga_ngram_freq["tetragram_freq"],))
        ...
        df['lex_suffix_score'] = df.apply(
            lambda row: calculate_suffix_score(row['lex_has_trusted_suffix'], row['lex_has_wellknown_suffix'],
                                               row["lex_has_cdn_suffix"],
